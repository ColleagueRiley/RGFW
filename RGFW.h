...
#ifdef RGFW_OPENGL
void RGFW_window_getVisual_OpenGL(RGFW_window* win, RGFW_glHints* hints) {
	...
	for (i = 0; i < fbcount; i++) {
		...
		if ((!(win->_flags & RGFW_windowTransparent) || vi->depth == 32) && samples <= hints->samples && samples > best_samples) {
			best_fbc = i;
			best_depth = vi->depth;
...
void RGFW_wl_pointer_motion(void* data, struct wl_pointer *pointer, u32 time, wl_fixed_t x, wl_fixed_t y) {
	...
	RGFW_eventQueuePushEx(e.type = RGFW_mousePosChanged;
									...
									e._win = win);
	win->_lastMouseX = (i32)wl_fixed_to_double(x);
	win->_lastMouseY = (i32)wl_fixed_to_double(y);
	RGFW_mousePosCallback(win, (i32)wl_fixed_to_double(x), (i32)wl_fixed_to_double(y), 0, 0);
}
...
void RGFW_wl_pointer_button(void* data, struct wl_pointer *pointer, u32 serial, u32 time, u32 button, u32 state) {
	...
	if (!(win->_enabledEvents & (RGFW_BIT(RGFW_mouseButtonReleased - RGFW_BOOL(state))))) return;
	u32 b = (button - 0x110);
...
void RGFW_wl_pointer_axis(void* data, struct wl_pointer *pointer, u32 time, u32 axis, wl_fixed_t value) {
	...
	if (!(win->_enabledEvents & (RGFW_BIT(RGFW_mouseScrollUp + (scroll < 0))))) return;

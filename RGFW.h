...
u32* RGFW_initFormatAttribs(u32 useSoftware) {
	...
	#define RGFW_GL_ADD_ATTRIB(attrib, attVal) \
		...
		if (RGFW_behavior == RGFW_releaseFlush) {
			RGFW_GL_ADD_ATTRIB(RGFW_GL_RELEASE_BEHAVIOR, RGFW_GL_CONTEXT_RELEASE);
		} else if (RGFW_behavior == RGFW_glReleaseNone) {
			RGFW_GL_ADD_ATTRIB(RGFW_GL_RELEASE_BEHAVIOR, RGFW_GL_CONTEXT_NONE);
		}

		u32 flags = 0;
		if (RGFW_CONTEXT_DEBUG) flags |= RGFW_GL_DEBUG_BIT;
		if (RGFW_CONTEXT_ROBUSTNESS) flags |= RGFW_GL_ROBUST_BIT;
...
void RGFW_createOpenGLContext(RGFW_window* win) {
...

	{
		...
		RGFW_GL_ADD_ATTRIB(EGL_RED_SIZE, RGFW_RED_SIZE);
		RGFW_GL_ADD_ATTRIB(EGL_GREEN_SIZE, RGFW_BLUE_SIZE);
		RGFW_GL_ADD_ATTRIB(EGL_BLUE_SIZE, RGFW_GREEN_SIZE);
		RGFW_GL_ADD_ATTRIB(EGL_ALPHA_SIZE, RGFW_ALPHA_SIZE);
		RGFW_GL_ADD_ATTRIB(EGL_DEPTH_SIZE, RGFW_DEPTH);
	
		if (RGFW_SRGB_CAPABLE)
			RGFW_GL_ADD_ATTRIB(0x3089, RGFW_SRGB_CAPABLE);
	...
	RGFW_GL_ADD_ATTRIB(EGL_CONTEXT_OPENGL_ROBUST_ACCESS, RGFW_CONTEXT_ROBUSTNESS);
	RGFW_GL_ADD_ATTRIB(EGL_CONTEXT_OPENGL_DEBUG, RGFW_CONTEXT_DEBUG);
	if (RGFW_behavior == RGFW_releaseFlush) {
...
u64 RGFW_getTime(void) {
	...
	clock_gettime(CLOCK_REALTIME, &ts);
	#endif
	unsigned long long int nanoSeconds = (u64)ts.tv_sec * 1000000000LLU + (u64)ts.tv_nsec;
...
void RGFW_window_setFullscreen(RGFW_window* win, RGFW_bool fullscreen) {
	...
	SetWindowPos(win->src.window, HWND_TOPMOST, 0, 0, mon.rect.w, mon.rect.h, SWP_NOOWNERZORDER | SWP_FRAMECHANGED);

	win->_oldRect = win->r;
	win->r = RGFW_RECT(0, 0, mon.rect.w, mon.rect.h);
}
...
u64 RGFW_getTime(void) { return (double)RGFW_getTimerValue() * RGFW_getTimerFreq(); }
u64 RGFW_getTimerFreq(void) { return (u64)RGFW_osx_initTimer(); }
...
u64 RGFW_getTimeNS(void) { return RGFW_getTimerValue(); }

u64 RGFW_getTime(void) {
	return emscripten_get_now() * 1000;
}

u64 RGFW_getTimerFreq(void) { return (u64)1000000000LLU; }

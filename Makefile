...
ifneq (,$(filter $(CC),cl /opt/msvc/bin/x64/cl.exe /opt/msvc/bin/x86/cl.exe))
	...
	NO_OSMESA ?= 1
	DX11_LIBS =
	WARNINGS += -Wno-missing-field-initializers -Wno-c++20-extensions -Wno-pedantic
	...
	ifeq ($(detected_OS),Darwin) 
		WARNINGS += -Wno-gnu-zero-variadic-macro-arguments -Wno-missing-field-initializers -Wno-c++20-extensions -Wno-pedantic
	endif
else
	...
	NO_VULKAN = 1
	ifeq ($(detected_OS),Darwin) 
		WARNINGS += -Wno-deprecated -Wno-unknown-warning-option -Wno-missing-field-initializers -Wno-c++20-extensions -Wno-pedantic
	endif
endif

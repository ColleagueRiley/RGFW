...
void checkEvents(RGFW_window* win) {
	...
	if (RGFW_isKeyPressed(win, RGFW_c) && (RGFW_isKeyPressed(win, RGFW_controlL) || RGFW_isKeyPressed(win, RGFW_controlR))) {
		char str[32] = {0};
		int size = snprintf(str, 32, "window %p: 刺猬", (void*)win);
		if (size > 0)
			RGFW_writeClipboard(str, (u32)size);
	}
	else if (RGFW_isKeyPressed(win, RGFW_v) && (RGFW_isKeyPressed(win, RGFW_controlL) || RGFW_isKeyPressed(win, RGFW_controlR))) {
		size_t len = 0;

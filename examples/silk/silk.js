...
var ASM_CONSTS = {
  71176: () => { Module.canvas.focus(); },  
 71201: () => { Module.useWebGL = true; GLImmediate.init(); },  
 71247: () => { window.addEventListener("keydown", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 1); }, true, ); },  
 71400: () => { window.addEventListener("keyup", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 0); }, true, ); },  
 ...
 72570: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 72641: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 72698: () => { return window.mouseX || 0; },  
 72729: () => { return window.mouseY || 0; },  
 72760: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 72907: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 72960: () => { return window.innerWidth; },  
 72990: () => { return window.innerHeight; }
};

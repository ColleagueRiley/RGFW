 ...
 * Porter: Riley Mabb <riley@hedgehogsoft.com>
 */
/*
...
static void demo_refresh_callback(const RGFW_event* e) {
    struct demo* demo = RGFW_window_getUserPtr(e->common.win);
    demo_draw(demo);
}

static void demo_resize_callback(const RGFW_event* e) {
    struct demo* demo = RGFW_window_getUserPtr(e->common.win);
    demo->width = e->update.w;
    demo->height = e->update.h;
    demo_resize(demo);
}

static void demo_run(struct demo *demo) {
    while (!RGFW_window_shouldClose(demo->window)) {
		RGFW_pollEvents();
        ...
        if (demo->frameCount != INT32_MAX && demo->curFrame == demo->frameCount)
            RGFW_window_setShouldClose(demo->window, RGFW_TRUE);
    }
}

static void demo_create_window(struct demo *demo) {
	demo->window = RGFW_createWindow(APP_LONG_NAME, 0, 0, demo->width, demo->height, RGFW_windowCenter);
    ...
    RGFW_window_setUserPtr(demo->window, demo);
    RGFW_setEventCallback(RGFW_windowRefresh, demo_refresh_callback);
    RGFW_setEventCallback(RGFW_windowResized, demo_resize_callback);
    RGFW_window_setExitKey(demo->window, RGFW_keyEscape);
}
...
static void demo_init_vk(struct demo *demo) {
    ...
    required_extensions = RGFW_getRequiredInstanceExtensions_Vulkan(&required_extension_count);
    if (!required_extensions) {
        ERR_EXIT("RGFW_getRequiredInstanceExtensions_Vulkan failed to find the "
...
static void demo_init_vk_swapchain(struct demo *demo) {
    ...
    RGFW_window_createSurface_Vulkan(demo->window, demo->inst, &demo->surface);

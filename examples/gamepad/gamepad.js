...
var ASM_CONSTS = {
  69640: () => { Module.canvas.focus(); },  
 69665: () => { Module.useWebGL = true; GLImmediate.init(); },  
 69711: () => { window.addEventListener("keydown", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 1); }, true, ); },  
 69864: () => { window.addEventListener("keyup", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 0); }, true, ); },  
 ...
 71034: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 71105: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 71162: () => { return window.mouseX || 0; },  
 71193: () => { return window.mouseY || 0; },  
 71224: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 71371: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 71424: () => { return window.innerWidth; },  
 71454: () => { return window.innerHeight; }
};

...
var ASM_CONSTS = {
  66960: () => { Module.canvas.focus(); },  
 66985: () => { Module.useWebGL = true; GLImmediate.init(); },  
 67031: () => { window.addEventListener("keydown", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 1); }, true, ); },  
 67184: () => { window.addEventListener("keyup", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 0); }, true, ); },  
 ...
 68354: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 68425: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 68482: () => { return window.mouseX || 0; },  
 68513: () => { return window.mouseY || 0; },  
 68544: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 68691: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 68744: () => { return window.innerWidth; },  
 68774: () => { return window.innerHeight; }
};

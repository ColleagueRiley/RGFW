...
var ASM_CONSTS = {
 ...
 420381: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 420452: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 420509: () => { return window.mouseX || 0; },  
 420540: () => { return window.mouseY || 0; },  
 420571: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 420718: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 420771: ($0, $1, $2, $3, $4) => { var data = Module.HEAPU8.slice($0, $0 + $1 * $2 * 4); let context = document.getElementById("canvas").getContext("2d"); let image = context.getImageData(0, 0, $1, $2); image.data.set(data); context.putImageData(image, 0, $4 - $2); },  
 421006: () => { return window.innerWidth; },  
 421036: () => { return window.innerHeight; },  
 421067: ($0, $1) => { var ext = UTF8ToString($0, $1); var canvas = document.querySelector('canvas'); var gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl'); if (!gl) return 0; var supported = gl.getSupportedExtensions(); return supported && supported.includes(ext) ? 1 : 0; },  
 421347: () => { Module.requestFullscreen(false, true); },  
 421386: () => { Module.exitFullscreen(false, true); },  
 421422: ($0, $1) => { var element = document.getElementById("canvas"); if (element) element.style.opacity = $1; }
};

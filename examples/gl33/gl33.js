...
var ASM_CONSTS = {
  67696: () => { Module.canvas.focus(); },  
 67721: ($0, $1, $2, $3, $4) => { var data = Module.HEAPU8.slice($0, $0 + $1 * $2 * 4); let context = document.getElementById("canvas").getContext("2d"); let image = context.getImageData(0, 0, $1, $2); image.data.set(data); context.putImageData(image, 0, $4 - $2); },  
 ...
 69940: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 70011: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 70068: () => { return window.mouseX || 0; },  
 70099: () => { return window.mouseY || 0; },  
 70130: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 70277: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 70330: ($0, $1) => { var ext = UTF8ToString($0, $1); var canvas = document.querySelector('canvas'); var gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl'); if (!gl) return 0; var supported = gl.getSupportedExtensions(); return supported && supported.includes(ext) ? 1 : 0; },  
 70610: () => { return window.innerWidth; },  
 70640: () => { return window.innerHeight; },  
 70671: () => { Module.requestFullscreen(false, true); },  
 70710: () => { Module.exitFullscreen(false, true); },  
 70746: ($0, $1) => { var element = document.getElementById("canvas"); if (element) element.style.opacity = $1; }
};

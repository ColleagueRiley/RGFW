...
var ASM_CONSTS = {
  68416: () => { Module.canvas.focus(); },  
 68441: ($0, $1, $2, $3, $4) => { var data = Module.HEAPU8.slice($0, $0 + $1 * $2 * 4); let context = document.getElementById("canvas").getContext("2d"); let image = context.getImageData(0, 0, $1, $2); image.data.set(data); context.putImageData(image, 0, $4 - $2); },  
 68676: () => { return window.devicePixelRatio || 1; },  
 68715: () => { return window.innerWidth || 0; },  
 68748: () => { return window.innerHeight || 0; },  
 ...
 69767: () => { var canvas = document.getElementById('canvas'); canvas.addEventListener('drop', function(e) { e.preventDefault(); if (e.dataTransfer.file < 0) return; var count = e.dataTransfer.files.length; var drop_dir = '.rgfw_dropped_files'; Module._RGFW_mkdir(drop_dir); for (var i = 0; i < count; i++) { var file = e.dataTransfer.files[i]; var path = '/' + drop_dir + '/' + file.name.replace("//", '_'); var reader = new FileReader(); reader.onloadend = (e) => { if (reader.readyState != 2) { out('failed to read dropped file: '+file.name+': '+reader.error); } else { var data = e.target.result; Module._RGFW_writeFile(path, new Uint8Array(data), file.size); } }; reader.readAsArrayBuffer(file); var filename = stringToNewUTF8(path); Module._Emscripten_onDrop(filename, path.length + 1); free(filename); } }, true); canvas.addEventListener('dragover', function(e) { e.preventDefault(); return false; }, true); },  
 70671: () => { return 'Asyncify' in Module; },  
 70704: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 70775: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 70832: () => { return window.mouseX || 0; },  
 70863: () => { return window.mouseY || 0; },  
 70894: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 71041: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 71094: () => { return window.innerWidth; },  
 71124: () => { return window.innerHeight; },  
 71155: () => { Module.requestFullscreen(false, true); },  
 71194: () => { Module.exitFullscreen(false, true); },  
 71230: ($0, $1) => { var element = document.getElementById("canvas"); if (element) element.style.opacity = $1; }
};
